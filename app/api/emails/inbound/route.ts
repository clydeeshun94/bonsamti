...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[v0] Received webhook request, body length:", rawBody.length)
    ...
    try {
      ...
      console.error("[v0] Failed to parse JSON payload:", parseError)
      return NextResponse.json({ error: "Invalid JSON payload" }, { status: 400 })
    }

    console.log("[v0] Parsed inbound email:", {
      from: payload.from,
      to: payload.to,
    ...
    if (!payload.from || !payload.to) {
      console.error("[v0] Invalid email payload: missing from or to fields")
    ...
    for (const recipientEmail of recipients) {
      try {
        console.log("[v0] Processing email for recipient:", recipientEmail)

        // Find the account associated with this email
        const account = await database.accounts.findByEmail(recipientEmail)

        if (!account) {
          console.log("[v0] No account found for email:", recipientEmail)
          results.push({
        ...
        if (accountAge > twentyFourHours) {
          console.log("[v0] Account expired for email:", recipientEmail)
          results.push({
            recipient: recipientEmail,
            status: "expired",
        ...
        console.log("[v0] Email stored successfully:", storedEmail.id)
        results.push({
          recipient: recipientEmail,
          status: "stored",
        ...
        console.error("[v0] Error processing email for recipient:", recipientEmail, error)
        results.push({
          recipient: recipientEmail,
          status: "error",
    ...
    console.log("[v0] Webhook processing complete:", response)
    return NextResponse.json(response)
  } catch (error) {
    console.error("[v0] Error processing inbound email webhook:", error)
